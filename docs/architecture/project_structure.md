...
```text
SManager/
  apps/                 # 跳板机运行的应用：后端、前端、Hermes-agent
...
`hermes-agent` 负责每日值守、轮询状态、补 FNL、自动重试和通知。
...
`diagnostics` 存放错误模式和恢复动作，供后端、Hermes-agent 和 AI 助手共用。
...
`flowctl.py` 是服务器唯一稳定入口，后端、Hermes-agent 和 AI 助手都通过它提交、查询、诊断和重试任务。
...
4. Hermes-agent 先做只读监控和通知，再开放自动重试。
